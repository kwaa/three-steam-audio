---
title: React Components
sidebar:
  order: 6
---

The `three-steam-audio/react` entry point provides declarative components for React Three Fiber.

## `<SteamAudio>`

Provider that owns or receives a [`World`](/reference/world/). It runs `world.step(delta)` automatically each frame.

```tsx twoslash
import { SteamAudio } from 'three-steam-audio/react'
```

### Props

`audioContext`
: Required when creating an owned world. The `AudioContext` to use.

`options`
: `Omit<WorldOptions, 'audioContext'>` — options passed to `createWorld`.

`world`
: An existing `World` to use instead of creating one.

`paused`
: `boolean` — when `true`, `world.step` is not called. Default: `false`.

`updatePriority`
: `number` — R3F `useFrame` priority. Default: `-100`.

`children`
: React nodes rendered inside the provider.

## `useSteamAudio()`

```ts twoslash
import { useSteamAudio } from 'three-steam-audio/react'

const { listener, scene, world } = useSteamAudio()
```

Returns the current world, listener, and scene. Must be called inside `<SteamAudio>`.

## `<SteamAudioListener>`

Synchronizes the R3F camera (or a custom object) to `world.listener`.

### Props

`object`
: Optional ref to an `Object3D` to use as the listener transform. Defaults to the R3F camera.

`perspectiveCorrection`
: `boolean | { factor?: number, applyInXR?: boolean }` — enables or configures the World's desktop perspective correction. Sources still opt in through `<SteamAudioSource perspectiveCorrection />` or `SourceSettings`.

## `<SteamAudioAmbisonicSource>`

Declarative Unity-compatible Ambisonic decoder. The wrapped group orientation is synchronized to the Ambisonic source each frame.

### Props

`input`
: `AudioNode | null` — a four-channel first-order AmbiX input.

`destination`
: `AudioNode | null` — where the stereo output is connected. Defaults to `world.audioContext.destination`.

`binaural`
: `boolean` — enables HRTF rendering. Defaults to `true`.

`settings`
: `AmbisonicSourceSettings` — the imperative source settings.

`onReady`
: `(api: SteamAudioAmbisonicSourceApi) => void` — called when the source group is mounted.

## `useSteamAudioAmbisonicSource(object, settings?)`

Lower-level hook that creates an Ambisonic source and decoder tied to an `Object3D` ref.

```ts
const { node, source } = useSteamAudioAmbisonicSource(objectRef)
```

## `<SteamAudioEnvironment>`

Creates optional reflection and reverb buses and wires them to a destination.

### Props

`destination`
: `AudioNode | null` — where buses are connected. Defaults to `world.audioContext.destination`.

`reflections`
: `boolean | ReflectionBusSettings` — enables and configures a reflection bus.

`reverb`
: `false | (ReverbBusSettings & ReverbSettings)` — enables and configures a reverb bus, and configures listener reverb.

`children`
: React nodes rendered inside the environment context.

## `useSteamAudioEnvironment()`

```ts twoslash
import { useSteamAudioEnvironment } from 'three-steam-audio/react'

const { reflectionBus, reverbBus } = useSteamAudioEnvironment()
```

Returns the buses created by the nearest `<SteamAudioEnvironment>`, if any.

## `<AcousticMesh>`

Wraps children meshes and registers them as acoustic geometry.

### Props

`material`
: `AcousticMaterial | readonly AcousticMaterial[] | ((mesh: Mesh) => AcousticMaterial | readonly AcousticMaterial[])` — material or materials to apply.

`dynamic`
: `boolean` — registers meshes as dynamic geometry. Default: `false`.

All other props are forwarded to the inner `<group>`.

:::warning
Does not support `SkinnedMesh` or morph targets.
:::

## `<SteamAudioSource>`

Declarative source component.

### Props

`input`
: `AudioNode | null` — the mono audio input.

`destination`
: `AudioNode | null` — where to connect the node's main output.

`airAbsorption`
: `DirectSettings['airAbsorption']` — `true` enables the default model; an object configures the exponential or curve model.

`direct`
: `SourceSettings['direct']`. Use this for full direct-path settings, including `mixLevel` and `transmission.maxSurfaces`.

`distanceAttenuation`
: `DirectSettings['distanceAttenuation']` — shortcut for the direct-path distance model.

`directMixLevel`
: `number` in `[0, 1]` — shortcut for `direct.mixLevel`.

`directOverrides`
: `DirectOverrides | null` — replaces selected direct simulation outputs. `null` clears all overrides.

`occlusion`
: `SourceSettings['direct']['occlusion']`.

`transmission`
: `'frequency-dependent' | 'frequency-independent' | boolean | DirectSettings['transmission']`. Use the object form to configure `maxSurfaces`.

`directivity`
: `{ dipoleWeight?: number, dipolePower?: number }`.

`spatialization`
: `SourceSettings['spatialization']`.

`reflections`
: `SourceSettings['reflections']`.

`reflectionsSend`
: `number` — gain to the environment reflection bus.

`reverbSend`
: `number` — gain to the environment reverb bus.

`settings`
: `SourceSettings` — full source settings object. Other props are merged into this object.

World HRTF settings are passed through `<SteamAudio options={{ hrtf: ... }}>`.

`onReady`
: `(api: SteamAudioSourceApi) => void` — called when the source group is mounted.

All other props are forwarded to the inner `<group>`.

## `useSteamAudioSource(object, settings?)`

Lower-level hook that creates a source and node tied to an `Object3D` ref.

```ts twoslash
import { useRef } from 'react'
import { useSteamAudioSource } from 'three-steam-audio/react'
import type { Object3D } from 'three'

const objectRef = useRef<Object3D>(null)
const { node, source } = useSteamAudioSource(objectRef)
```

## `SteamAudioSourceApi`

```ts twoslash
import type { Group } from 'three'
import type { SteamAudioNode } from 'three-steam-audio'
import type { Source } from 'three-steam-audio'

interface SteamAudioSourceApi {
  group: Group
  node: SteamAudioNode
  source: Source
}
```
